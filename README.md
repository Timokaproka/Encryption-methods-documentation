...
Главная цель — помочь начинающим программистам освоить шифрование данных на языке программирования Python и предостеречь их от совершения частых ошибок. 
...
- **tag** (байты) - параметр который используется для проверки целостности данных (только для расшифровки)
...
Для шифрования и расшифрования в большинстве режимов используются методы класса encrypt() и decrypt() соответственно, в которые предаются открытый текст и шифротекст соответственно, результатом и там и там будет байт-строка. 
Исключением являются CCM, EAX, GCM, SIV, OCB, которые используют методы encrypt_and_digest() и decrypt_and_verify() для шифрования и расшифрования соответственно. В encrypt_and_digest передаётся открытый текст, результатом является **кортеж**, нулевым элементом которого будет **шифротекст** (байт-строка), а первым - **tag** (байт-строка), который используется для проверки целостности данных, а в decrypt_and_verify шифротекст и tag, который использовался для расшифровки текста, результатом будет байт-строка.
...
Для шифрования и расшифровки ECB используется методы созданного экземпляра класса encrypt и decrypt. 
...
Для шифрования и расшифровки CBC используется методы созданного экземпляра класса encrypt и decrypt. 
...
Для шифрования и расшифровки CFB используется методы созданного экземпляра класса encrypt и decrypt, но так как режим делает блочный шифр поточным, то не надо использовать функции pad() и unpad(). 
...
Для шифрования и расшифровки OFB используется методы созданного экземпляра класса encrypt и decrypt, но так как режим делает блочный шифр поточным, то не надо использовать функции pad() и unpad(). 
...
2. IV для шифрования и IV для расшифровки - разные. IV для шифрования может быть задан или сгенерирован библиотекой, но IV для расшифровки берётся из данных, после шифрования. 
...
# Для шифрования текста
cipher = AES.new(key, AES.MODE_SIV, nonce=nonce)
ciphertext, tag = cipher.encrypt_and_digest(text) # функция возвращает кортеж, в котором нулевой элемент - шифротекст, а первый - тег (MAC)
...
> #### ВАЖНО!
> Если вы шифруете или расшифровывайте несколько фрагментов, то в конце вы ДОЛЖНЫ вызвать encrypt или decrypt без параметров! 
```Python
from Crypto.Cipher import AES
from Crypto.Random import get_random_bytes
...
Пример кода для AES-128 (для разового шифрования и расшифровки):
```Python
from Crypto.Cipher import AES
from Crypto.Random import get_random_bytes
...
Пример кода для AES-128 (для шифрования и расшифровки фрагментами):
```Python
from Crypto.Cipher import AES
from Crypto.Random import get_random_bytes
...
for i in range(2):
    ...
    start = end 
plaintext += decipher.decrypt() # ЗАВЕРШЕНИЕ РАСШИФРОВКИ ВЫЗОВОМ пустого decrypt
...
2. При вызове decrypt(), первым аргументом нужно передавать зашифрованный текст, а вторым то, чему будет равна переменная в случае неудачной расшифровки.
...
plaintext = decipher.decrypt(ciphertext, "Error!") # вторым аргументом передаётся, то, чему будет равна переменная в случае неудачной расшифровки.
```
...
В этом разделе мы рассмотрим ошибки, которые вы можете допустить при работе с шифрами.
...
4. Отказ от тестирования на правильность шифрования и расшифровки.
...
- Проводите тщательное тестирование шифрования и расшифровки, чтобы убедиться в правильности работы вашей программы.